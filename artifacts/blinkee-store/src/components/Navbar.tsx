...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-background/80 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4 ml-4 pl-4 border-l border-white/10">
            <Link href="/cart">
              <Button variant="ghost" size="icon" className="relative hover:bg-white/5 hover:text-primary transition-colors">
                ...
                {cartItemCount > 0 && (
                  <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-secondary text-[10px] font-bold text-white shadow-[0_0_8px_rgba(255,0,170,0.8)]">
                    {cartItemCount}
                  </span>
                )}
              </Button>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-background/95 backdrop-blur-xl border-b border-white/10 p-4 flex flex-col gap-4 shadow-[0_10px_30px_rgba(0,0,0,0.8)] animate-in slide-in-from-top-2">
          <Link 
            href="/" 
            onClick={closeMenu}
